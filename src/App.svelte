<script lang="ts">
  ...
  function exportSession(session: Session) {
    const data = JSON.stringify({ id: session.id, title: session.title, file: session.file, workspace: workspacePath }, null, 2)
    ...
    <div class="app-grid" style={`grid-template-columns:${showLeft ? 220 : 0}px minmax(430px, 1fr) ${showRight ? rightWidth : 0}px`}>
      ...
      {#if sessionMenu}
        <div class="session-menu-backdrop" role="presentation" on:click={() => (sessionMenu = null)}>
          <div class="session-menu" style={`left:${sessionMenu.x}px; top:${sessionMenu.y}px`} role="menu" on:click|stopPropagation>
            ...
            <button on:click={() => void chooseWorkspace()}>设置工作区</button>
